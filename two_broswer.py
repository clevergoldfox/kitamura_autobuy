...
class CustomWindow(QWidget):
    ...
    def _continue_monitoring_with_browser(self, products):
        """Window-based monitoring that purchases immediately for all detected links."""
        try:
            if self.shop_page is None:
                self.add_log("エラー: ブラウザを利用できません")
                return
            ...
            self.add_log(f"監視URL: {FIXED_MONITORING_URL}")

            all_processed_urls = set()
            MAX_PROCESSED_URLS = 500

            # Detection windows (start_minute, end_minute). Ranges are inclusive and wrap across hours when start > end.
            DETECTION_WINDOWS = [(59, 1), (14, 16), (34, 36)]
            PAGE_REFRESH_INTERVAL = 1.0  # seconds between checks inside a window

            if not DETECTION_WINDOWS:
                self.add_log("警告: 検知ウィンドウが設定されていません。監視を終了します。")
                return

            def in_window(now):
                m = now.minute
                for start_min, end_min in DETECTION_WINDOWS:
                    if start_min <= end_min:
                        if start_min <= m <= end_min:
                            return True
                    else:  # wraps the hour (e.g., 59-1)
            ...
            def current_window_end(now):
                m = now.minute
                for start_min, end_min in DETECTION_WINDOWS:
                    if start_min <= end_min and start_min <= m <= end_min:
                        return now.replace(minute=end_min, second=59, microsecond=0)
                    if start_min > end_min and (m >= start_min or m <= end_min):
                        end_time = now
                        if m <= end_min:
                            end_time = now + timedelta(hours=1)
                        return end_time.replace(minute=end_min, second=59, microsecond=0)
                return now + timedelta(seconds=1)

            def next_window_start(now):
                candidates = []
                for start_min, _ in DETECTION_WINDOWS:
                    candidate = now.replace(minute=start_min, second=0, microsecond=0)
                    if candidate <= now:
                        candidate += timedelta(hours=1)
                    candidates.append(candidate)
                return min(candidates)

            self.add_log("開始: 時間帯監視モード (59-01,14-16,34-36)")
            ...
            while self.monitoring_active:
                try:
                    ...
                    if not in_window(now):
                        ...
                        self.update_last_log(f"待機: 次の検知枠 {next_start.strftime('%H:%M:%S')} まで {wait_seconds}秒")
                        while self.monitoring_active and datetime.now() < next_start:
                            try:
                                task = self.browser_task_queue.get(timeout=0.5)
                                if task and task.get('type') == 'speed_test':
                                    self._execute_speed_test_task(task['url'])
                            except queue.Empty:
                                pass
                            remaining = max(0, int((next_start - datetime.now()).total_seconds()))
                            self.update_last_log(f"待機: 次の検知枠 {next_start.strftime('%H:%M:%S')} まで {remaining}秒")
                            time.sleep(1)
                        continue

                    # We are inside a detection window
                    window_end = current_window_end(datetime.now())
                    refresh_count = 0

                    while self.monitoring_active:
                        # CRITICAL: Check time at the start of each loop iteration
                        # This ensures .exe builds respect the time window
                        now = datetime.now()
                        if not in_window(now) or now >= window_end:
                            # Window ended, exit immediately
                            break
                        
                        # Recalculate window_end in case we moved to a different window
                        current_window = current_window_end(now)
                        if current_window != window_end:
                            window_end = current_window
                        
                        try:
                            task = self.browser_task_queue.get(timeout=0.01)
                            if task and task.get('type') == 'speed_test':
                                self._execute_speed_test_task(task['url'])
                                continue
                        except queue.Empty:
                        ...
                        # Double-check time before expensive page load
                        now = datetime.now()
                        if not in_window(now) or now >= window_end:
                            break

                        refresh_count += 1
                        self.shop_page.goto(FIXED_MONITORING_URL, wait_until="domcontentloaded")

                        # Check time again after page load
                        now = datetime.now()
                        if not in_window(now) or now >= window_end:
                            break

                        remaining_window = max(0, int((window_end - now).total_seconds()))
                        self.update_last_log(f"検知中... 残り{remaining_window}秒（{refresh_count}回目）")
                        ...
                        if product_detail_links:
                            ...
                            if new_products:
                                # Immediately log discoveries and start purchasing sequentially
                                self.add_log(f"{len(new_products)}件の対象商品を検知 → 購入開始")
                                for idx, detail_link in enumerate(new_products, 1):
                                    # Check time before each purchase (critical for .exe builds)
                                    now = datetime.now()
                                    if not in_window(now) or now >= window_end:
                                        self.add_log("⚠ 監視時間が終了しました。購入を中断します")
                                        break
                                    
                                    if not self.monitoring_active:
                                        break
                                    self.add_log(f"購入開始 {idx}/{len(new_products)}: {detail_link}")
                                    ...
                                    if len(all_processed_urls) > MAX_PROCESSED_URLS:
                                        urls_to_remove = list(all_processed_urls)[:100]
                                        for url in urls_to_remove:
                                            all_processed_urls.discard(url)

                                    if success:
                                        self.add_log("購入成功")
                                    
                                    # Check time after purchase
                                    now = datetime.now()
                                    if not in_window(now) or now >= window_end:
                                        self.add_log("⚠ 監視時間が終了しました")
                                        break
                                
                                # Final check before continuing loop
                                now = datetime.now()
                                if not in_window(now) or now >= window_end:
                                    break
                                continue
                    ...
                    if self.monitoring_active:
                        self.update_last_log(f"サイクル完了 ({refresh_count}回チェック)")

                except Exception:
                    time.sleep(1)

            self.add_log("終了: 監視を停止しました")

        except Exception as e:
            self.add_log(f"監視中エラー: {str(e)}")
            import traceback
    ...
    def __init__(self):
        ...
        self.shop_browser = None  # Playwright browser
        self.shop_context = None  # Playwright context
        self.shop_page = None     # Playwright page
    ...
    def start_manual_auth_browser(self):
        ...
        try:
            ...
            try:
                ...
                self.add_log(">")
                self.add_log("🚀 自動監視を開始します")
                self.add_log(">")

                # Continue with monitoring logic (browser is already open)
    ...
    def closeEvent(self, event):
        ...
        try:
            ...
            # Properly close Playwright resources to avoid EPIPE errors
            try:
                if self.shop_browser:
    ...
    def extract_product_links_by_filter_fast(self, page, product_filters):
